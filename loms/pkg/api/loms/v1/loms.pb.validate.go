...
func (m *OrderInfoResponse) validate(all bool) error {
	...
	if _, ok := _OrderInfoResponse_Status_InLookup[m.GetStatus()]; !ok {
		err := OrderInfoResponseValidationError{
			field:  "Status",
			reason: "value must be in list [NEW CANCELLED]",
		}
		if !all {
			return err
		}
...
var _OrderInfoResponse_Status_InLookup = map[Statuses]struct{}{
	0: {},
	4: {},
}
